...
void RCSwitch::send(unsigned long remote, unsigned long button, boolean onoff){
	this->send( remote, false, button, onoff);
}

void RCSwitch::send(unsigned long remote, boolean group, unsigned long button, boolean onoff){
		if (this->nProtocol != 4){
			Serial.println("function not availble in this protocol");
			return;
		}
		
    int repeatMessage = this->nRepeatTransmit;
    setRepeatTransmit(1);
    for (int nRepeat=0; nRepeat<repeatMessage; nRepeat++) {
    ...
    setRepeatTransmit(repeatMessage);
}
